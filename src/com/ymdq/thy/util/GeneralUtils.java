...
import com.ymdq.thy.bean.community.WebUrlParameters;
...
public final class GeneralUtils
{
    ...
    public static Typeface getFontFace(Context contx)
    {
        if(isNull(Constants.fontFace))
        {
            Constants.fontFace = Typeface.createFromAsset(contx.getAssets(),
                "fonts/H-GungSeo2.2.ttf");
        }
        return Constants.fontFace;
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    public static List<WebUrlParameters> getWebUrl(String content)
    {
        List<WebUrlParameters> parameters = new ArrayList<WebUrlParameters>();
        WebUrlParameters parameter;
        String str =
        ...
        while (m.find())
        {
            parameter = new WebUrlParameters(m.group(), m.start(), m.end());
            parameters.add(parameter);
        }
        return parameters;
    }
    
    public static String ConstructionData(String content, List<WebUrlParameters> parameters)
    {
        if (GeneralUtils.isNullOrZeroSize(parameters))
            return content;
        String temp;
        String str = "";
        for (int i = 0; i < parameters.size(); i++)
        {
            if (i == 0)
            {
                temp = content.substring(0, parameters.get(i).getStart());
            }
            else
            {
                temp = content.substring(parameters.get(i - 1).getEnd(), parameters.get(i).getStart());
            }
            if (GeneralUtils.isNotNullOrZeroLenght(temp))
                str += temp;
            str += "网页链接";
        }
        str += content.substring(parameters.get(parameters.size() - 1).getEnd(), content.length());
        return str;
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    public static SpannableStringBuilder clickSpan(TextView view, final Context context, String content,
        final List<WebUrlParameters> parameters)
    {
        List<WebUrlParameters> newParameters = new ArrayList<WebUrlParameters>();
        SpannableStringBuilder ssb = new SpannableStringBuilder();
        ssb.append(ConstructionData(content, parameters));
        if (GeneralUtils.isNullOrZeroSize(parameters))
            return ssb;
        ...
        String str = "网页链接";
        Pattern p = Pattern.compile(str);
        Matcher m = p.matcher(ConstructionData(content, parameters));
        while (m.find())
        {
            parameter = new WebUrlParameters(m.group(), m.start(), m.end());
        ...
        for (int i = 0; i < newParameters.size(); i++)
        {
            ...
            ssb.setSpan(new ClickableSpan()
            {
                ...
                public void onClick(View widget)
                {
                    ...
                    intent.putExtra("wev_view_url", parameters.get(position).getUrl());
                ...
                public void updateDrawState(TextPaint ds)
                {
                    ...
                    ds.setColor(context.getResources().getColor(R.color.community_web_url));
                }
            }, newParameters.get(i).getStart(), newParameters.get(i).getEnd(), 0);
        }
        return ssb;
    }
